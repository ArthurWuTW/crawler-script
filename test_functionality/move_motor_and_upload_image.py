...
class ImageGrabber():
    ...
    def resetCaputringConfiguration(id):
        self.grabber = cv2.VideoCapture(id)
...
class ArucoLibrary():
    ...
    def getCenterPoints(corners, ids):
        centerPoints = list()
        for i in range(len(ids)):
            ...
            if(len(c[:.0]) == 4):
                ...
                centerPointsArray.append(centerPoint)
        return centerPoints

def ImagePoster():
    def __init__(self, url):
        self.url = url
    def postImageArucoid(image, id):
